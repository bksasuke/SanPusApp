...
- (void)presentPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    ...
    if (self.popupViewController == nil) {
         [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notificationForKeyBoard:) name:UIKeyboardWillShowNotification object:nil];
        // initial setup
...
-(void)notificationForKeyBoard:(NSNotification*)noti{
    NSDictionary* keyboardInfo = [noti userInfo];
    NSValue* keyboardFrameBegin = [keyboardInfo valueForKey:UIKeyboardFrameEndUserInfoKey];
    CGRect keyboardFrameBeginRect = [keyboardFrameBegin CGRectValue];
    int heightKeyBoard=keyboardFrameBeginRect.size.height;
    self.popupViewController.view.frame=CGRectMake(self.popupViewController.view.frame.origin.x, self.view.frame.size.height-heightKeyBoard-self.popupViewController.view.frame.size.height-10, self.popupViewController.view.frame.size.width, self.popupViewController.view.frame.size.height);
}
@end
